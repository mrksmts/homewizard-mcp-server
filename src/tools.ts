...
import type { DeviceInfo, Measurement, SystemStatus } from "./types.js";
...
export function registerTools(server: McpServer, client: HomeWizardClient): void {
  registerGetDeviceInfo(server, client);
  registerGetMeasurement(server, client);
  registerGetTelegram(server, client);
  registerGetSystemStatus(server, client);
}

// ---- 1. device info ----------------------------------------------------

function registerGetDeviceInfo(server: McpServer, client: HomeWizardClient): void {
  server.registerTool(
    "homewizard_get_device_info",
    {
      title: "Get HomeWizard Device Info",
      description: `Return identifying information about the HomeWizard device behind HOMEWIZARD_HOST: product type (e.g. "HWE-P1" for the P1 meter), product name, serial number, firmware version, and supported API version.
  ...
  - You want live energy readings — use homewizard_get_measurement.`,
    ...
    async ({ response_format }): Promise<ToolContent> => {
      try {
        ...
        return ok(formatDeviceInfo(info, response_format));
      } catch (error) {
        return err(describeApiError(error, "homewizard_get_device_info"));
      }
    },
  );
}

function formatDeviceInfo(info: DeviceInfo, format: "markdown" | "json"): string {
...
function registerGetMeasurement(server: McpServer, client: HomeWizardClient): void {
  server.registerTool(
    "homewizard_get_measurement",
    {
      title: "Get Live HomeWizard Measurement",
      description: `Read the most recent live measurement from the P1 meter. Returns instantaneous power draw (W), per-phase voltages/currents, frequency, and CUMULATIVE energy totals (kWh since the meter was installed) plus any external sub-meters (gas, water).
  ...
  - Negative power_w / power_l*_w means power flowing OUT to the grid (export).`,
    ...
    async ({ response_format }): Promise<ToolContent> => {
      try {
        ...
        return ok(formatMeasurement(m, response_format));
      } catch (error) {
        return err(describeApiError(error, "homewizard_get_measurement"));
      }
    },
  );
}

function formatMeasurement(m: Measurement, format: "markdown" | "json"): string {
  if (format === "json") {
    return JSON.stringify(stripNulls(m), null, 2);
  }
...
function registerGetTelegram(server: McpServer, client: HomeWizardClient): void {
  server.registerTool(
    "homewizard_get_telegram",
    {
      title: "Get Raw P1 Telegram",
      description: `Return the raw DSMR telegram string most recently received from the smart meter. This is the unprocessed text the meter pushes to the P1 port (OBIS codes, line by line, ending with a CRC). Useful for debugging or for clients that prefer to parse DSMR directly.
...
Only available on the P1 meter (HWE-P1).`,
    ...
    async (): Promise<ToolContent> => {
      try {
        ...
        return ok(telegram.length > CHARACTER_LIMIT
...
function registerGetSystemStatus(server: McpServer, client: HomeWizardClient): void {
  server.registerTool(
    "homewizard_get_system_status",
    {
      title: "Get HomeWizard System Status",
      description: `Read the device's system state: Wi-Fi network name and signal strength, uptime, cloud-communication status, status-LED brightness, and whether the v1 local API is currently enabled (v2 only).
  ...
  - Confirming Wi-Fi connectivity quality.`,
    ...
    async ({ response_format }): Promise<ToolContent> => {
      try {
        ...
        return ok(formatSystemStatus(s, response_format, client.apiVersion));
      } catch (error) {
        return err(describeApiError(error, "homewizard_get_system_status"));
      }
    },
  );
}
...
function formatSystemStatus(
  ...
  if (format === "json") return JSON.stringify({ ...s, api_version_in_use: apiVersion }, null, 2);
